...
from editregions.contrib.embeds.models import Iframe, JavascriptAsset, StylesheetAsset, Feed
...
class IframeTestCase(DjangoTestCase):
    ...
    def test_get_name(self):
        iframe = Iframe(position=2, region='test', content_type_id=1,
                        content_id=1, url='https://news.bbc.co.uk/',
                        name='BBC News')
        iframe.full_clean()
        iframe.save()
...
class FeedTestCase(DjangoTestCase):
    def test_str(self):
        feed = Feed(position=2, region='test', content_type_id=1,
                    content_id=1, url='http://example.com/')
...
class JavascriptAssetTestCase(DjangoTestCase):
    ...
    def test_cleaning_no_data(self):
        asset = self.model(position=2, region='test', content_type_id=1,
                           content_id=1)
        try:
            asset.full_clean()
    ...
    def test_cleaning_ok_data(self):
        asset = self.model(position=2, region='test', content_type_id=1,
                           content_id=1, local='x/y')
        asset.full_clean()

    def test_cleaning_both_data(self):
        asset = self.model(position=2, region='test', content_type_id=1,
                           content_id=1, local='x/y', external='a/z')
        try:
            asset.full_clean()
        except ValidationError as e:
            self.assertEqual(len(e.message_dict), 1)
    ...
    def test_external_scheme_relative_if_possible(self):
        ...
        asset2 = self.model(position=2, region='test', content_type_id=1,
